...
# ==========================================
# IMPOR MEDIAPIPE BARU (STANDAR PYTHON 3.13)
# ==========================================
...
# inisialisasi Model Bentuk Hati   
...
# ==========================================
# FUNGSI LOGIKA DETEKSI GESTUR (PERBAIKAN STRUKTUR DATA)
# ==========================================
def hitung_mode_gestur_baru(landmarks):
    # Mengambil koordinat y dari setiap ujung dan sendi jari
    # Di API baru, objek landmark diakses dengan indeks langsung landmark[i]
    tips = [8, 12, 16, 20]  
    pips = [6, 10, 14, 18]
    
    # Deteksi apakah jari sedang tegak/berdiri
    jari_berdiri = [landmarks[t].y < landmarks[p].y for t, p in zip(tips, pips)]
...
# ==========================================
# THREAD BACKGROUND: PROSES KAMERA & AI (PERBAIKAN DATA PASSING)
# ==========================================
def camera_thread_func():
    ...
    while shared_data["running"]:
        ...
        if results.hand_landmarks and len(results.hand_landmarks) > 0:
            ...
            # PERBAIKAN DI SINI: Ambil ukuran resolusi frame kamera yang asli secara dinamis
            h_frame, w_frame, _ = frame.shape
            
            # Gambar marker titik tepat di posisi jari
            # 1. Simpan semua koordinat piksel sendi ke dalam list
            points = []
            for lm in hand_landmarks:
                cx, cy = int(lm.x * w_frame), int(lm.y * h_frame)
                points.append((cx, cy))

            # 2. Struktur daftar koneksi garis rangka sesuai standar MediaPipe
            connections = [
                ...
                (0, 17)                                # Telapak Tangan Bawah
            ]
            
            # 3. Gambar garis rangka putih
            for start, end in connections:
                cv2.line(frame, points[start], points[end], (255, 255, 255), 2)

            # 4. Gambar titik sendi merah muda (Pinkish Red)
            for pt in points:
                cv2.circle(frame, pt, 5, (100, 100, 255), -1)
                
            local_mode = hitung_mode_gestur_baru(hand_landmarks)
...
# ==========================================
# MAIN THREAD: RENDERING GRAFIS 3D
# ==========================================
pygame.init()
pygame.display.set_mode((WIDTH, HEIGHT), DOUBLEBUF | OPENGL)
pygame.display.set_caption("Space Gesture Controller - Python 3.13 Fixed")
